...
use super::{
    ...
    read_join_hello, reconcile_phase_after_disconnect, remove_lobby_player, rename_lobby_player,
    reset_race_from_lobby, return_to_lobby, set_lobby_ai_difficulty, unique_lobby_name,
...
use crate::net::protocol::{
    ClientMessage, LobbyPlayer, PlayerKind, RaceResultStatus, ServerMessage, decode_server_message,
...
fn joiner_disconnect_is_removed_from_waiting_lobby_with_event() {
    ...
    let closed_game = handle_player_disconnect(&mut state, PlayerId(2), std::time::Instant::now());
...
fn host_disconnect_sends_game_closed_to_joiners() {
    ...
    let closed_game = handle_player_disconnect(&mut state, PlayerId(1), std::time::Instant::now());
