...
- The host presses `Space` to start with ready connected racers. This can be a solo host race if no joiners or AIs are ready. Unready players stay in the lobby and can ready for the next race.
...
- If no connected racers remain during countdown, the game returns to the lobby.
